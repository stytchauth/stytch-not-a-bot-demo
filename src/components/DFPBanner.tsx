...
import { stytchFingerprintLookup } from "../app/shared-actions";
import { getTelemetryId } from "../app/utils";

export default function DFPBanner() {
  useEffect(() => {
    async function checkDFP() {
      const telemetryId = await getTelemetryId();
      const result = await stytchFingerprintLookup({
